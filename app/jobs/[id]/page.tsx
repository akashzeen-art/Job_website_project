...
export default async function JobDetailPage({ params }: PageProps<"/jobs/[id]">) {
  ...
  const similar = (await getJobs())
    .filter(
      (item) =>
        item.id !== job.id &&
        (item.department === job.department || item.city === job.city),
    )
    .slice(0, 3);
  ...
  return (
    <div className="mx-auto grid max-w-6xl gap-8 px-4 py-8 sm:px-6 sm:py-12 lg:grid-cols-[minmax(0,1fr)_280px]">
      <article>
        <Link href="/jobs" className="text-sm text-muted hover:text-text">
          ← Board
        </Link>
          ...
          <div className="flex items-start gap-3 sm:gap-4">
            ...
            <div className="min-w-0 flex-1">
              <p className="text-xs tracking-[0.14em] text-muted uppercase">
                <Link href={`/companies/${job.companySlug}`} className="hover:text-text">
                  {job.company}
                </Link>
                {company ? ` · ${company.hq}` : ""}
              </p>
              <h1 className="mt-2 font-display text-[1.85rem] leading-tight sm:text-4xl">
                {job.title}
              </h1>
      ...
      <aside className="hidden h-fit space-y-4 lg:sticky lg:top-24 lg:block">
        ...
        <div className="border border-line bg-surface p-5">
          ...
          <p className="mt-5 text-xs leading-5 text-muted">
            Applications stay on the company’s own site.
          </p>
        </div>
      </aside>
    </div>
