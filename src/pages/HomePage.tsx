    ...
    <div>
          ...
          <div className="mt-8 max-w-2xl">
            ...
            <div className="no-scrollbar mt-3 flex gap-2 overflow-x-auto pb-1">
              {PROMPTS.slice(0, 4).map((prompt) => (
                <Link
                  key={prompt}
                  to={`/jobs?q=${encodeURIComponent(prompt)}`}
      ...
      <section className="mx-auto max-w-6xl px-4 py-12 sm:px-6 sm:py-16">
        <div className="mb-6 flex items-end justify-between gap-4">
          ...
          <Link to="/jobs" className="shrink-0 text-sm text-gold">
            The board
          </Link>
        </div>
      ...
      <section className="mx-auto grid max-w-6xl gap-8 px-4 py-12 sm:px-6 sm:py-16 md:grid-cols-3 md:gap-10">
        <Step n="I" title="Ask plainly" body="A city, a house, a craft. Meridian reads the sentence and keeps India-based desks." />
        <Step n="II" title="Open with taste" body="Fit is recency, hub, and house — a quiet score, not a résumé theatre." />
        <Step n="III" title="Apply at source" body="Greenhouse, Ashby, Workday. No second account. No résumé left here." />
      </section>
    </div>
  );
}
